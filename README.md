...
Presets live in subfolders of `Documents\MCPresets`, one folder per
preset, mirroring the layout of an instance's `minecraft` folder:
...
A preset works best on instances close to the version it was built from.
...
Naming presets after what they were built for, such as `fabric-1.21-sodium`
...
Create a preset root folder if you do not have one already, then
pick an instance and a preset from the dropdowns. Once you're done, click **Apply
...
The preset name is optional and defaults to `base`. Pass `--dry-run` to list
...
- [Create Preset From Instance](https://github.com/HatTapper/mc-settings-sync/issues/1): Allow users to create a preset folder from an existing launcher instance by picking
